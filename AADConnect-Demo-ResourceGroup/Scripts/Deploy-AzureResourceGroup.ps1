...
Param(
  ...
  [string] $ResourceGroupName = 'AADConnect-Demo-Solution',  
...
if ($UploadArtifacts)
{
	...
	# Create the DSC Configuration Zip file
	$DSCSourceFolder = [System.IO.Path]::Combine($PSScriptRoot, $DSCSourceFolder)
	if (Test-Path $DSCSourceFolder)
	{
		Add-Type -Assembly System.IO.Compression.FileSystem
		$DSCArchiveFile = $ArtifactStagingDirectory + "\dsc.zip"
		if (Test-Path $DSCArchiveFile)
		{
			Remove-Item $DSCArchiveFile
		}
		[System.IO.Compression.ZipFile]::CreateFromDirectory($DSCSourceFolder, $DSCArchiveFile)
	}
	...
	if ($LASTEXITCODE -ne 0) {
	    Write-Error "azcopy failed with return code: $LASTEXITCODE"
		return
	} 
